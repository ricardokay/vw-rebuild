...
- **Lightbox on ink (2026-09-14):** the gallery/article lightbox is **WordPress 7.1 core's image-block lightbox** (Interactivity API; markup `wp-includes/blocks/image.php`, CSS `wp-includes/blocks/image/style.css`), not Newspack's. With no `theme.json`, core printed an inline white scrim at a 0.9 resting opacity that only its motion-enabled zoom lifts to 1, so with reduced motion the page ghosted through. `gallery.css` now sets the scrim to `--vw-ink`, opaque at rest in both motion paths (one `!important`, against the inline style); light (`--vw-bg-subtle`) close/prev/next icons, set on the svg because the parent forces `color: unset !important` on the close button; 44px close; a light focus ring. **iOS follow-up (2026-09-14):** overlay `height: 100dvh` (100vh fallback — core's 100vh is iOS Safari's large viewport, which pushed prev/next under the bottom toolbar), an ink `box-shadow` spill past the overlay box, prev/next and the credit pill offset by `env(safe-area-inset-bottom)`, and the scroll lock moved from `html` overflow:hidden to **body `position: fixed; top: -scrollY`**, set and undone by `vw-lightbox-caption.js` (exact position restored on close). **Whole photo opens it (2026-09-14):** core already binds the tap to the `<img>` (`data-wp-on--click="actions.showLightbox"`, cursor zoom-in); on touch screens (`hover: none`) core's hover reveal of the corner button is switched off so iOS delivers the first tap as a click, not a hover — no JS delegation (would double-fire). Headless-verified only; **pending Ricardo's iPhone check** (dark ground, arrows, scroll lock, one-tap open).
...
- **Mobile nav LIVE (2026-09-14, mobile round part 1, option B):** below 960px the inline nav collapses into a fixed **light** bottom bar of MODES — Home / Sections / Search / Archive — on the page ground with a 1px top hairline plus a faint 5% upward shadow, ink icons, red for the current mode; and a full-height **dark** panel on the footer palette (`vw_footer_palette_vars()`). **The panel is the "everything" drawer (2026-09-14, variant P2):** the seven sections plus **The Archive** as an eighth row (active on `/archive/`; still a bar tab too), a hairline, then the footer's **About set** as a recessive 2-column grid (Inter 13px, 70% opacity, 44px rows). Search field in Search mode. **One registry, two renderers:** `VW_FOOTER_ABOUT` in `inc/footer.php` is the only list; `vw_footer_link_url()` is the single published-status rule used by the footer (`vw_footer_link_item()`) and the panel (`vw_mnav_about_item()`), and an unpublished page degrades to dimmed plain text in both. `VW_MOBILE_PANEL_LINKS` is deleted. The drawer fits 375×812 unscrolled in Sections mode and scrolls inside the panel on shorter screens. **Compact mobile masthead (follow-up, variant A2):** one 56px row — staggered thin-line hamburger left, 170px wordmark centred, search right (44px each) — no dateline, no heavy rule, a 1px hairline; the hamburger and search icon open the SAME panel as the bar (`[data-vw-mnav-open]`, one implementation); `vw_masthead_brand_render()` is shared by header and homepage; at ≥960px the row is `display: contents` so desktop is unchanged. Without JS the hamburger is hidden (inline nav showing) and search is a link to `/?s=`. Body gets bar-height + `env(safe-area-inset-bottom)` padding, `viewport-fit=cover` is set in `header.php`. **Must See Films is the seventh nav item** (desktop + mobile; its front is the standard `lead-3col` PHP part since 2026-09-14). **Active item underlined at every width** (2px, red stays but no longer carries the state alone); below 960px a centred **"you are here"** line under the masthead carries it on section fronts and category archives — **not on single articles** (2026-09-14: the article kicker already names the section; `vw_masthead_here_render()` returns early on `is_singular('post')`). **44px tap targets** on nav, bar, panel and every footer link (desktop nav via zero-layout hit-area padding — masthead height unchanged). No-JS fallback: `html.vw-js` gates the collapse, so without JavaScript the wrapped inline nav stays. Files: `inc/masthead.php`, `assets/css/masthead-nav.css`, `assets/js/masthead-nav.js`.
- **Mobile round part 2 LIVE (2026-09-14):**
  - **Homepage lead (gate pick L1):** below 1024px the lead photo runs full-bleed directly under the header row, with the headline BELOW it (never overlaid), a soft 5% seam shadow and a 3-line dek.
  - **The lead is a chrome setting:** `vw_chrome['mobile_lead']`, either `image-first` (the default, not stored) or `standard` (text-first). It is read by `vw_chrome_mobile_lead()`, and the field sits in the chrome panel. Per-section variants are out of scope. **Standard mode spans the full column below 1024px** (2026-09-14 fix: a stale 24px side padding on `.vwh2-lead` had inset it to 295px at 375). The `vw_chrome` row exists from panel saves; as last read (2026-09-14) it stores `slogan: "Independent Since 2012"` and **`mobile_lead: "standard"`**, so the live homepage lead is text-first.
  - **Focused search mode (gate pick S2), below 960px:**
    - the header has a ✕ close (history back when arriving from this site, otherwise home) and a 112px wordmark; no sections button or search icon;
    - a sticky 56px serif field with an ink underline, which gains its hairline only when stuck;
    - a result count printed from `loop_start` (no fork of `search.php`);
    - the bottom bar is kept; desktop search is unchanged.
  - **Articles below 783px:** every header case is left-aligned; headline 30–34px; body 18px / 1.55 with a 24px paragraph gap.
  - **One 16px gutter below 600px** on articles, archive, search and section fronts, aligned to the masthead line.
  - **Deks (all widths, all zones):** Georgia in `--vw-dek-ink`.
  - **Section fronts below 1024px:** 24/19/18px headlines; Browse-all becomes a numeral plus a sentence, with a 44px CTA.
  - **/archive/ below 600px:** 30px title; 20px rows with 3-line excerpts and a 96px square thumbnail on the right.
  - **Tall case-A article images** (square or portrait, 9 posts) are capped at 720px tall, centred.
  - **Colours:** 33 literals moved to palette tokens (`--vw-mark-*`, `--vw-band-*`, `--vw-dek-ink`, `--vw-shadow-soft`, `--vw-card-placeholder`).
...
- **~104 duplicate-title pairs** archive-wide (14 in Photography) — distinct post IDs, display-deduped on fronts, **pending editorial review**; two pairs have conflicting dates. **The front dedupe seed misses quote-variant pairs** (one copy stores curly quotes, the other straight; `wptexturize` renders them identically but `vw_older_duplicate_ids()` compares raw titles): 6 pairs on fronts — must-see-films 4 (14377, 1634, 1637, 9902; three show twice on page 1), a-la-music 1 (5066), photography 1 (67571). Gated fix pending (normalize the title key; changes those three fronts).
- **48 posts carry scraped comment chrome** in `post_content` (deks cleaned display-layer only).
- `must-see-films` front aligned 2026-09-14 (standard part, cats 15 + 16 netflix-films); only 8 of its 169 posts carry a usable image, so the lead is an older (2020) story. Film section mark deferred. Section term names are lowercase for photography, food drink, out n about, must see films (screen-reader h1 + document title).
- Archive page layout is consistent but **undesigned** (mobile rows tightened 2026-09-14; desktop untouched).
- Articles: a second hairline sits under the credit strip before the body at every width (pre-existing, from body content/Newspack, not the header); the author-bio name under articles renders as a very large sans heading (pre-existing). `article-header.css` / `inc/article-header.php` header comments still say "PREVIEW ONLY" — stale since the 2026-09-12 rollout.
...
- Implemented: `category.php` asks `vw_tpl_section_part()` (`inc/templates.php`) for the term. A front renders only when the term has a template assigned (term meta `_vw_tpl`, set from the Edit Category dropdown) AND `section-parts/{slug}.php` exists — included directly. Legacy `section-parts/{slug}.html` (rendered via `do_blocks()`) is still honoured by the resolver, but **no `.html` part remains** (`must-see-films.html` retired 2026-09-14). Everything else — no template, no part, or `is_paged()` — falls through to the Newspack archive.
- Fronts with templates (`_vw_tpl = lead-3col`): `a-la-music` (umbrella, queries 6 music category IDs: 7,9,8,11,20,10), `photography` (6), `food-drink` (13 + 14 hungry-social), `out-n-about` (17), `political-megaphone` (18), `book-reviews` (30), `must-see-films` (15 + 16 netflix-films). Page-1 queries use explicit `category__in` lists (direct assignments only); page 2+ is the native archive and includes child categories.
