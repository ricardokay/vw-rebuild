...
- Decision: junk-author suppression is applied in `vw_credits_inline()`, not in the hoisted `vw_ah_credits()`, so the approved article-header preview stays byte-identical. The header still prints desk-label authors — logged for the Round 8 rollout rather than changed silently inside an approved design. **Closed 2026-09-12 in the rollout round: `vw_ah_credits()` now omits the By line for junk authors and the duplicate rule in `vw_credits_inline()` was deleted, so one function answers for both surfaces.**
...
**September 12, 2026 — rollout round: masthead + article header go live (`d417a70`)**
- **Status change: both designs are LIVE, no longer preview-gated.** The v2 masthead is the header on every surface; the adaptive article header is the header on every single post.
- Decision: both shipped by **template override, not CSS suppression** — `header.php` for the masthead, a child `template-parts/header/entry-header.php` for the article header. The latter is the part `single.php` and all four `large-featured-image.php` branches already request, so every single-post layout is covered without forking `single.php`. A sibling `entry-header-newspack.php` requires the parent file, because a child part cannot `get_template_part()` the file it shadows.
- Decision: preview flags are retained as no-ops rather than deleted (`vw_masthead_active()` returns `false`), so old bookmarked `?vw_masthead=1` links degrade instead of fatalling.
- Decision: the case-A threshold is **1200**, which makes it a pure no-upscale rule rather than a taste call — the content column tops out at 1200px, so case A is chosen only when the source can fill it without stretching.
- Decision: photo-led posts **suppress the header image** when the featured attachment also appears among the first three images of the body, and fall to the case-C stack. The picture is not removed, only un-duplicated; a reader should not meet the same frame twice in two lines.
- **Lesson worth keeping: presence is not rendering.** The prior session's server-side sweep asserted the header markup was present, and it was — but two defects were only visible on a painted page. (1) The active nav item had never actually been red: its rule sat in `section-landing.css` at the same specificity (0,1,0) as the base ink colour in `homepage-v2.css`, which is enqueued last. It was invisible during preview because the masthead only ever appeared on the homepage, where no item is active. (2) The article header rendered at 0×0 on **desktop and mobile alike**, because the preview-era `.vw-ah-single .entry-header { display: none }` now hid the very element our header renders inside. Every sweep from here asserts computed geometry, not markup presence.
- Verified: 16 surfaces × 1440 and 390 — masthead present, `.vw-nav` absent, SVG wordmark, zero horizontal overflow, red `rgb(196,18,48)` active nav where one applies, six article cases resolving A/B/C/C-dedup/B/B with exactly one `.vw-ah` and no parent hero per page. Twelve suites green.
