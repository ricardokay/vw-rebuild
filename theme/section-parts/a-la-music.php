<?php
/**
 * A La Music section front — four-zone custom render.
...
$music_cats = [ 7, 9, 8, 11, 20, 10 ]; // a-la-music, live-music-reviews, album-reviews, music-interviews, music-videos, music-editorials
$used_ids   = [];

// Shared excerpt helper — works outside the loop.
...
// Center anchor: prefer a sticky music post with a real image on disk.
// Falls back to most recent post with any real image (tier ≥ 1).
$anchor = null;
...
// Second center story (no image guard — rendered text-only below the anchor).
$anchor2 = null;
$q = new WP_Query( [
	'category__in'   => $music_cats,
...
// Left list: 5 most recent posts, excluding anchor.
$left_posts = [];
$q = new WP_Query( [
	'category__in'   => $music_cats,
	'post__not_in'   => $used_ids,
...
// Right list: 5 more posts, excluding anchor + left.
$right_posts = [];
$q = new WP_Query( [
	'category__in'   => $music_cats,
	'post__not_in'   => $used_ids,
...
if ( $anchor ) :
	...
	$anchor_cat = vw_primary_cat_name( $anchor->ID, $music_cats );
		...
		<div class="vw-module__inner">
			<div class="vw-lead-block">
				...
				<div class="vw-lead-block__col vw-lead-block__col--center">
					...
					<?php if ( $anchor2 ) :
						$a2_cat = vw_primary_cat_name( $anchor2->ID, $music_cats );
					?>
				...
				<div class="vw-lead-block__col vw-lead-block__col--left">
					...
					<ul class="vw-lead-block__list">
						<?php foreach ( $left_posts as $p ) :
							$lcat = vw_primary_cat_name( $p->ID, $music_cats );
						?>
							<li>
								<?php if ( $lcat ) : ?>
				...
				<div class="vw-lead-block__col vw-lead-block__col--right">
					...
					<ul class="vw-lead-block__list">
						<?php foreach ( $right_posts as $p ) :
							$rcat = vw_primary_cat_name( $p->ID, $music_cats );
						?>
							<li>
								<?php if ( $rcat ) : ?>
...
if ( $feat_posts ) {
	...
	$lead_cat  = vw_primary_cat_name( $lead->ID, $music_cats );
	?>
	<div class="vw-module">
		<div class="vw-module__inner">
			<div class="vw-feat-list">
				...
				<ul class="vw-feat-list__items">
					<?php foreach ( $feat_posts as $p ) :
						$item_cat = vw_primary_cat_name( $p->ID, $music_cats );
					?>
		...
		<div class="vw-module__inner">
			<ul class="vw-hl-list vw-hl-list--2col">
				<?php foreach ( $hl_posts as $p ) :
					$hl_cat = vw_primary_cat_name( $p->ID, $music_cats );
				?>
					<li>
		...
		<div class="vw-module__inner">
			<div class="vw-grid">
				<?php foreach ( $grid_posts as $p ) :
					...
					$card_cat = vw_primary_cat_name( $p->ID, $music_cats );
					$is_text  = ( $tier === 0 );
				?>
